...
async def handle_stripe_webhook(
    ...
    if event_type == "checkout.session.completed":
        ...
        # Handle both events - invoice.paid is standard, but some configs send invoice.payment_succeeded
...
async def handle_invoice_paid(invoice: dict):
    ...
    # Debug: log full invoice structure for subscription-related fields
    print(f"[INVOICE DEBUG] id={invoice_id}")
    print(f"[INVOICE DEBUG] subscription={invoice.get('subscription')}")
    print(f"[INVOICE DEBUG] billing_reason={invoice.get('billing_reason')}")
    print(f"[INVOICE DEBUG] amount_paid={invoice.get('amount_paid')}")

    # Check line items for subscription if not at top level
    lines = invoice.get("lines", {}).get("data", [])
    if lines:
        for i, line in enumerate(lines):
            print(f"[INVOICE DEBUG] line[{i}].subscription={line.get('subscription')}")
            print(f"[INVOICE DEBUG] line[{i}].type={line.get('type')}")
            print(f"[INVOICE DEBUG] line[{i}].amount={line.get('amount')}")

    # Check if this is a subscription invoice - try top level first, then line items
    subscription_id = invoice.get("subscription")

    # If not at top level, try to get from line items
    if not subscription_id and lines:
        for line in lines:
            if line.get("subscription"):
                subscription_id = line.get("subscription")
                print(f"[INVOICE] Found subscription in line items: {subscription_id}")
                break

    # For subscription_update invoices, fetch full invoice from Stripe API if subscription not found
    billing_reason = invoice.get("billing_reason")
    if not subscription_id and billing_reason == "subscription_update":
        print(f"[INVOICE] Fetching full invoice from Stripe for subscription_update")
        try:
            full_invoice = stripe.Invoice.retrieve(invoice_id)
            subscription_id = full_invoice.get("subscription")
            print(f"[INVOICE] Got subscription from API: {subscription_id}")
            # Use the full invoice data for processing
            if subscription_id:
                invoice = full_invoice
        except Exception as e:
            print(f"[INVOICE] Failed to fetch full invoice: {e}")
    ...
    print(f"[INVOICE] billing_reason={billing_reason}, amount_paid={amount_paid}, invoice_id={invoice_id}")

    if billing_reason == "subscription_create":
        print(f"Skipping initial subscription invoice {invoice_id}")
        return

    # Handle subscription updates (upgrades, billing switches) separately
    if billing_reason == "subscription_update":
        print(f"[INVOICE] Processing subscription_update invoice")
        await handle_subscription_update_invoice(invoice)
        return
...
async def handle_subscription_update_invoice(invoice: dict):
    """
    Handle invoice.paid for subscription updates (upgrades, billing switches).

    This creates accurate transaction records with real payment data from Stripe.
    Credits are NOT added here - they're handled by the endpoint for upgrades.

    Uses pending_action metadata from subscription to determine transaction type.
    """
    invoice_id = invoice.get("id")
    subscription_id = invoice.get("subscription")

    if not subscription_id:
        print(f"No subscription ID in update invoice {invoice_id}")
        return
    ...
    if list(existing_txn):
        print(f"[IDEMPOTENCY] Update invoice {invoice_id} already processed")
        return
    ...
    if not pending_action:
        print(f"No pending_action in subscription metadata for invoice {invoice_id}")
        return
    ...
    if not user_id:
        print(f"No user_id in subscription {subscription_id}")
        return
    ...
    # Get card details from charge if available
    card_last4 = None
    card_brand = None
    charge_id = invoice.get("charge")
    if charge_id:
    ...
    # Determine transaction type and build data based on pending_action
    if pending_action == "upgrade":
        tx_type = "subscription_upgrade"
        old_tier = metadata.get("old_tier", "unknown")
        new_tier = metadata.get("new_tier", "unknown")
        credits_added = int(metadata.get("credits_added", 0))
        balance_before = int(metadata.get("balance_before", 0))
        balance_after = int(metadata.get("balance_after", 0))
        description = f"Upgraded from {old_tier.capitalize()} to {new_tier.capitalize()}"
        if credits_added > 0:
    ...
    elif pending_action.startswith("billing_switch"):
        tx_type = pending_action  # billing_switch_annual or billing_switch_monthly
        tier = metadata.get("tier", "unknown")
        is_annual = "annual" in pending_action
        description = f"Switched to {'annual' if is_annual else 'monthly'} billing"
        ...
        print(f"Unknown pending_action: {pending_action}")
        return

    # Add card details to metadata
    if card_last4:
        txn_data["metadata"]["card_last4"] = card_last4
    if card_brand:
        txn_data["metadata"]["card_brand"] = card_brand

    # Create the transaction
    db.collection("credit_transactions").add(txn_data)
    print(f"Created {tx_type} transaction for user {user_id}: ${amount_cents/100:.2f}")

    # Clear pending_action metadata from subscription
    try:
        ...
        print(f"Failed to clear subscription metadata: {e}")
