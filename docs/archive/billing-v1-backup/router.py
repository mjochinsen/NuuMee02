...
async def upgrade_subscription(
    ...
    try:
        # Get user data and calculate credits FIRST (before Stripe call)
        user_ref = db.collection("users").document(user_id)
        user_doc = user_ref.get()
        credits_added = 0
        current_balance = 0
        ...
        # SINGLE modify call with BOTH price change AND metadata
        # This ensures metadata is set BEFORE webhook fires
        # Use always_invoice to charge immediately (not defer to next billing cycle)
...
async def switch_billing_period(
    ...
    try:
        ...
        # Include pending_action metadata for webhook to create accurate transaction
        # Use always_invoice to charge immediately (not defer to next billing cycle)
        updated_sub = stripe.Subscription.modify(
            ...
            metadata={
                "billing_period": billing_period,
                "pending_action": f"billing_switch_{'annual' if switch_request.annual else 'monthly'}",
        ...
        # Transaction will be created by invoice.paid webhook with accurate amount/receipt
