...
export default function SignupPage() {
  ...
  function getPasswordStrength(pwd: string) {
    ...
    if (score >= 75) return { score, label: 'Strong', color: 'bg-green-500' };
    if (score >= 50) return { score, label: 'Good', color: 'bg-blue-500' };
    if (score >= 25) return { score, label: 'Fair', color: 'bg-amber-500' };
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#0F172A] via-[#1E293B] to-[#0F172A] flex items-center justify-center px-6 py-12">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="border border-[#334155] rounded-2xl p-8 bg-[#0F172A]/80 backdrop-blur-sm">
          <div className="mb-6">
            <h1 className="text-[#F1F5F9] text-2xl mb-2">Create Your Account</h1>
            <p className="text-[#94A3B8]">Sign up to get 25 free credits</p>
          </div>
          ...
          <form onSubmit={handleEmailSignup} className="space-y-4">
            <div>
              <label htmlFor="fullName" className="block text-[#94A3B8] text-sm mb-2">
                Full Name
              </label>
              <input
                id="fullName"
            ...
            <div>
              <label htmlFor="confirmPassword" className="block text-[#94A3B8] text-sm mb-2">
                Confirm Password
              </label>
              <input
                id="confirmPassword"
          ...
          <p className="text-center text-[#94A3B8] text-sm mt-6">
            ...
            <Link href="/login" className="text-[#00F0D9] hover:text-[#00F0D9]/80">
              Sign in
            </Link>
          </p>
